...
from app.bot.services.llm_response_processing import process_general_llm_response
...
async def process_general_input(message: Message):
    ...
    if not get_user_from_cache(message.from_user.id):
        ...
        answer = await process_general_llm_response(response=response, chat_id=message.from_user.id)
